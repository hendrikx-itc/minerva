...
CREATE FUNCTION "trend_directory"."create_base_table"(trend_directory.trend_store_part, trend_directory.table_trend[])
...
CREATE FUNCTION "trend_directory"."define_table_trends"(trend_directory.trend_store_part, trend_directory.trend_descr[])
    RETURNS trend_directory.trend_store_part
AS $$
INSERT INTO trend_directory.table_trend(name, data_type, trend_store_part_id, description, time_aggregation, entity_aggregation) (
    SELECT name, data_type, $1.id, description, time_aggregation, entity_aggregation FROM unnest($2)
);
...
CREATE FUNCTION "trend_directory"."define_trend_store"("data_source_name" text, "entity_type_name" text, "granularity" interval, "partition_size" integer)
    RETURNS trend_directory.trend_store
AS $$
...
CREATE FUNCTION "trend_directory"."define_trend_store_part"("trend_store_id" integer, "name" name, "trends" trend_directory.trend_descr[])
    RETURNS trend_directory.trend_store_part
AS $$
...
CREATE FUNCTION "trend_directory"."define_trend_store"(trend_directory.trend_store, trend_directory.trend_store_part_descr[])
    RETURNS trend_directory.trend_store
AS $$
SELECT trend_directory.define_trend_store_part($1.id, name, trends)
FROM unnest($2);
...
CREATE FUNCTION "trend_directory"."define_trend_store"("data_source_name" text, "entity_type_name" text, "granularity" interval, "partition_size" integer, "trends" trend_directory.trend_store_part_descr[])
    RETURNS trend_directory.trend_store
AS $$
...
CREATE FUNCTION "trend_directory"."create_trend_store"("data_source_name" text, "entity_type_name" text, "granularity" interval, "partition_size" integer, "parts" trend_directory.trend_store_part_descr[])
    RETURNS trend_directory.trend_store
AS $$
...
BEGIN
    ...
    EXECUTE format(
        'INSERT INTO trend.%I (entity_id, timestamp, created, job_id, %s) SELECT entity_id, timestamp, now(), 0, %s FROM %s WHERE timestamp = $1',
...
BEGIN
    ...
    EXECUTE format(
        'INSERT INTO trend.%I (entity_id, timestamp, created, job_id, %s) SELECT entity_id, timestamp, now(), 0, %s FROM %s($1)',
        (trend_directory.dst_trend_store_part($1)).name,
        columns_part,
        columns_part,
...
ALTER TABLE "trend_directory"."trend_view_part"
  ADD CONSTRAINT "trend_view_part_trend_store_id_fkey"
  FOREIGN KEY (trend_store_id)
