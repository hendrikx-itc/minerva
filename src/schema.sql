...
COMMENT ON COLUMN "trend_directory"."view_materialization"."processing_delay" IS 'The time after the destination timestamp before this materialization
can be executed';

COMMENT ON COLUMN "trend_directory"."view_materialization"."stability_delay" IS 'The time to wait after the most recent modified timestamp before
the source data is considered ''stable''';

COMMENT ON COLUMN "trend_directory"."view_materialization"."reprocessing_period" IS 'The maximum time after the destination timestamp that the materialization
is allowed to be executed';

COMMENT ON COLUMN "trend_directory"."view_materialization"."enabled" IS 'Indicates if jobs should be created for this materialization (manual
...
COMMENT ON COLUMN "trend_directory"."function_materialization"."processing_delay" IS 'The time after the destination timestamp before this materialization
can be executed';

COMMENT ON COLUMN "trend_directory"."function_materialization"."stability_delay" IS 'The time to wait after the most recent modified timestamp before
the source data is considered ''stable''';

COMMENT ON COLUMN "trend_directory"."function_materialization"."reprocessing_period" IS 'The maximum time after the destination timestamp that the materialization
is allowed to be executed';

COMMENT ON COLUMN "trend_directory"."function_materialization"."enabled" IS 'Indicates if jobs should be created for this materialization (manual
execution is always possible)';
...
COMMENT ON TABLE "trend_directory"."view_materialization_state" IS 'ID of the most recent job for this materialization';
...
COMMENT ON COLUMN "trend_directory"."view_materialization_state"."processed_fingerprint" IS 'Snapshot of the source_fingerprint at the time of
...
COMMENT ON TABLE "trend_directory"."view_materialization_trend_store_link" IS 'Stores the dependencies between a view materialization and its source table trend
...
of the source (table_trend_store_part) to a timestamp of the target (the
view and target table_trend_store_part).';
...
CREATE FUNCTION "trend_directory"."transfer"("source" trend_directory.view_trend_store, "target" trend_directory.table_trend_store, "timestamp" timestamp with time zone, "trend_names" text[])
...
BEGIN
    ...
    EXECUTE format(
        ...
        trend_directory.base_table_name(target),
        columns_part,
        columns_part,
