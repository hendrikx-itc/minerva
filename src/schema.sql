...
CREATE UNIQUE INDEX "table_trend_store_unique_constraint" ON "trend_directory"."table_trend_store" USING btree (entity_type_id, data_source_id, granularity);
...
CREATE UNIQUE INDEX "view_trend_store_unique_constraint" ON "trend_directory"."view_trend_store" USING btree (entity_type_id, data_source_id, granularity);
...
BEGIN
    PERFORM (WITH mapped_modified AS (
      SELECT
        view_materialization_id,
        trend_directory.map_timestamp(view_materialization_trend_store_link, NEW.timestamp) AS dst_timestamp
      FROM trend_directory.view_materialization_trend_store_link
      WHERE table_trend_store_part_id = NEW.table_trend_store_part_id
    )
    SELECT trend_directory.update_source_fingerprint(vm, dst_timestamp)
    FROM mapped_modified
    JOIN trend_directory.view_materialization vm ON vm.id = view_materialization_id
    GROUP BY vm, dst_timestamp);
...
COMMENT ON COLUMN "trend_directory"."view_materialization"."dst_trend_store_part_id" IS 'The ID of the destination table_trend_store_part';

COMMENT ON COLUMN "trend_directory"."view_materialization"."processing_delay" IS 'The time after the destination timestamp before this materialization can be executed
';

COMMENT ON COLUMN "trend_directory"."view_materialization"."stability_delay" IS 'The time to wait after the most recent modified timestamp before the source data is considered ''stable''
';

COMMENT ON COLUMN "trend_directory"."view_materialization"."reprocessing_period" IS 'The maximum time after the destination timestamp that the materialization is allowed to be executed
';

COMMENT ON COLUMN "trend_directory"."view_materialization"."enabled" IS 'Indicates if jobs should be created for this materialization (manual execution is always possible)
';

CREATE UNIQUE INDEX "ix_view_materialization_uniqueness" ON "trend_directory"."view_materialization" USING btree (dst_trend_store_part_id);



CREATE FUNCTION "trend_directory"."source_fingerprint_sql"(trend_directory.view_materialization)
    RETURNS text
AS $$
SELECT format('SELECT * FROM trend.%I($1)', $1::text || '_fingerprint');
$$ LANGUAGE sql STABLE;

COMMENT ON FUNCTION "trend_directory"."source_fingerprint_sql"(trend_directory.view_materialization) IS 'Returns the query to generate fingerprints for the specified view materialization.';


CREATE FUNCTION "trend_directory"."source_fingerprint"(trend_directory.view_materialization, timestamp with time zone)
...
COMMENT ON FUNCTION "trend_directory"."source_fingerprint"(trend_directory.view_materialization, timestamp with time zone) IS 'Returns the fingerprint of the combined states of all sources required to calculate the data for the target timestamp.
';
...
COMMENT ON COLUMN "trend_directory"."function_materialization"."dst_trend_store_id" IS 'The ID of the destination table_trend_store';

COMMENT ON COLUMN "trend_directory"."function_materialization"."processing_delay" IS 'The time after the destination timestamp before this materialization can be executed
';

COMMENT ON COLUMN "trend_directory"."function_materialization"."stability_delay" IS 'The time to wait after the most recent modified timestamp before the source data is considered ''stable''
';

COMMENT ON COLUMN "trend_directory"."function_materialization"."reprocessing_period" IS 'The maximum time after the destination timestamp that the materialization is allowed to be executed
';

COMMENT ON COLUMN "trend_directory"."function_materialization"."enabled" IS 'Indicates if jobs should be created for this materialization (manual execution is always possible)
';

CREATE UNIQUE INDEX "ix_function_materialization_uniqueness" ON "trend_directory"."function_materialization" USING btree (dst_trend_store_id);
...
CREATE TABLE "trend_directory"."view_materialization_state"
(
...
COMMENT ON TABLE "trend_directory"."view_materialization_state" IS 'Stores the relation between the state of the sources used for the materialization and the state of the materialized data, so that from this table, it can be decided if a new materialization should be done.
';

COMMENT ON COLUMN "trend_directory"."view_materialization_state"."materialization_id" IS 'The ID of the materialization type';

COMMENT ON COLUMN "trend_directory"."view_materialization_state"."timestamp" IS 'The timestamp of the materialized (materialization result) data';

COMMENT ON COLUMN "trend_directory"."view_materialization_state"."source_fingerprint" IS 'Aggregate state of all sources';

COMMENT ON COLUMN "trend_directory"."view_materialization_state"."processed_fingerprint" IS 'Snapshot of the source_fingerprint at the time of the most recent materialization
';

COMMENT ON COLUMN "trend_directory"."view_materialization_state"."job_id" IS 'ID of the most recent job for this materialization';

GRANT SELECT ON TABLE "trend_directory"."view_materialization_state" TO minerva;

GRANT INSERT,UPDATE,DELETE ON TABLE "trend_directory"."view_materialization_state" TO minerva_writer;



CREATE FUNCTION "trend_directory"."update_source_fingerprint"(trend_directory.view_materialization, timestamp with time zone)
    RETURNS void
AS $$
INSERT INTO trend_directory.view_materialization_state(materialization_id, timestamp, source_fingerprint, processed_fingerprint, job_id)
VALUES ($1.id, $2, (trend_directory.source_fingerprint($1, $2)).body, null, null)
ON CONFLICT ON CONSTRAINT view_materialization_state_pkey DO UPDATE SET source_fingerprint = (trend_directory.source_fingerprint($1, $2)).body;
$$ LANGUAGE sql VOLATILE;

COMMENT ON FUNCTION "trend_directory"."update_source_fingerprint"(trend_directory.view_materialization, timestamp with time zone) IS 'Update the fingerprint of the sources in the view_materialization_state table.';
...
CREATE TABLE "trend_directory"."view_materialization_trend_store_link"
(
  "view_materialization_id" integer NOT NULL,
  "table_trend_store_part_id" integer NOT NULL,
  "timestamp_mapping_func" regprocedure NOT NULL,
  PRIMARY KEY (view_materialization_id, table_trend_store_part_id)
);

COMMENT ON TABLE "trend_directory"."view_materialization_trend_store_link" IS 'Stores the dependencies between a view materialization and its source table trend store parts. Multiple levels of views and functions may exist between a materialization and its source table trend stores. These intermediate views and functions are not registered here, but only the table trend stores containing the actual source data used in the view.
The timestamp_mapping_func column stores the function to map a timestamp of the source (table_trend_store_part) to a timestamp of the target (the view and target table_trend_store_part).
';

COMMENT ON COLUMN "trend_directory"."view_materialization_trend_store_link"."view_materialization_id" IS 'Reference to a view_materialization.';

COMMENT ON COLUMN "trend_directory"."view_materialization_trend_store_link"."table_trend_store_part_id" IS 'Reference to a table_trend_store_part that is a source of the materialization referenced by view_materialization_id.
';

COMMENT ON COLUMN "trend_directory"."view_materialization_trend_store_link"."timestamp_mapping_func" IS 'The function that maps timestamps in the source table to timestamps in the materialized data. For example, for a view for an hour aggregation from 15 minute granularity data will need to map 4 timestamps in the source to 1 timestamp in the resulting data.
';



CREATE FUNCTION "trend_directory"."map_timestamp"(trend_directory.view_materialization_trend_store_link, timestamp with time zone)
    RETURNS timestamp with time zone
AS $$
DECLARE
    result timestamp with time zone;
BEGIN
    EXECUTE format('SELECT %s($1)', $1.timestamp_mapping_func::regproc::name) INTO result USING $2;

    RETURN result;
END;
$$ LANGUAGE plpgsql VOLATILE;

COMMENT ON FUNCTION "trend_directory"."map_timestamp"(trend_directory.view_materialization_trend_store_link, timestamp with time zone) IS 'Map timestamp using the mapping function defined in the link';
...
CREATE FUNCTION "trend_directory"."create_base_table_sql"("name" text, trend_directory.table_trend[])
    RETURNS text[]
AS $$
SELECT ARRAY[
    format(
    ...
    format(
        'SELECT create_hypertable(''%I.%I'', ''timestamp'', chunk_time_interval => interval ''1 day'');',
        trend_directory.base_table_schema(),
        name
    )
];
...
CREATE FUNCTION "trend_directory"."create_base_table"("name" name, trend_directory.table_trend[])
    RETURNS name
AS $$
...
CREATE FUNCTION "trend_directory"."create_base_table"(trend_directory.table_trend_store_part, trend_directory.table_trend[])
...
CREATE FUNCTION "trend_directory"."get_trend_store_part_trends"(trend_directory.table_trend_store_part)
    RETURNS trend_directory.table_trend[]
AS $$
SELECT COALESCE(array_agg(table_trend), ARRAY[]::trend_directory.table_trend[])
...
CREATE FUNCTION "trend_directory"."define_table_trend"("trend_store_part_id" integer, "name" name, "data_type" text, "description" text)
    RETURNS trend_directory.table_trend
AS $$
INSERT INTO trend_directory.table_trend (table_trend_store_part_id, name, data_type, description)
VALUES ($1, $2, $3, $4)
...
CREATE FUNCTION "trend_directory"."show_trends"(trend_directory.table_trend_store_part)
    RETURNS SETOF trend_directory.trend_descr
AS $$
SELECT
...
WHERE
    n.nspname = 'trend' AND
    a.attisdropped = false AND
    a.attnum > 0 AND table_trend.table_trend_store_part_id = $1.id;
$$ LANGUAGE sql STABLE;
...
CREATE FUNCTION "trend_directory"."define_table_trends"(trend_directory.table_trend_store_part, trend_directory.trend_descr[])
    RETURNS trend_directory.table_trend_store_part
AS $$
INSERT INTO trend_directory.table_trend(name, data_type, table_trend_store_part_id, description) (
...
CREATE FUNCTION "trend_directory"."staged_timestamps"("part" trend_directory.table_trend_store_part)
...
SET name = $3
WHERE table_trend_store_part_id = $1.id AND name = $2;
...
LEFT JOIN trend_directory.table_trend ON table_trend.name = required.name AND table_trend.table_trend_store_part_id = $1.id
...
CREATE FUNCTION "trend_directory"."get_trends_for_trend_store_part"("trend_store_part_id" integer)
    RETURNS SETOF trend_directory.table_trend
AS $$
SELECT * FROM trend_directory.table_trend WHERE table_trend.table_trend_store_part_id = $1;
$$ LANGUAGE sql STABLE;


CREATE FUNCTION "trend_directory"."get_trends_for_trend_store_part"(trend_directory.table_trend_store_part)
    RETURNS SETOF trend_directory.table_trend
AS $$
...
CREATE FUNCTION "trend_directory"."trend_store_part_has_trend_with_name"("part" trend_directory.table_trend_store_part, "trend_name" name)
    RETURNS bool
AS $$
SELECT exists(
    SELECT 1
...
CREATE FUNCTION "trend_directory"."get_max_modified"(trend_directory.table_trend_store, timestamp with time zone)
    RETURNS timestamp with time zone
AS $$
DECLARE
...
CREATE FUNCTION "trend_directory"."update_modified"("table_trend_store_part_id" integer, "timestamp" timestamp with time zone, "modified" timestamp with time zone)
    RETURNS void
AS $$
INSERT INTO trend_directory.modified AS m (table_trend_store_part_id, timestamp, start, "end")
VALUES ($1, $2, $3, $3)
ON CONFLICT ON CONSTRAINT modified_pkey DO UPDATE SET "end" = greatest(m."end", EXCLUDED."end");
$$ LANGUAGE sql VOLATILE;
...
WITH process_log AS (
  SELECT
    max(id) AS max_id,
    trend_directory.update_modified(table_trend_store_part_id, timestamp, max(modified)) AS update
...
CREATE FUNCTION "trend_directory"."clear_table_trend_store_part_sql"("table_trend_store_part_id" integer)
    RETURNS text
AS $$
SELECT format('DELETE FROM trend.%I WHERE timestamp = $1', ttsp.name)
FROM trend_directory.table_trend_store_part ttsp WHERE id = $1;
$$ LANGUAGE sql VOLATILE;

COMMENT ON FUNCTION "trend_directory"."clear_table_trend_store_part_sql"("table_trend_store_part_id" integer) IS 'Return the query to remove all records of the specified timestamp from the table_trend_store_part.
';


CREATE FUNCTION "trend_directory"."clear_table_trend_store_part"("table_trend_store_part_id" integer, "timestamp" timestamp with time zone)
    RETURNS integer
AS $$
DECLARE
    row_count integer;
BEGIN
    EXECUTE trend_directory.clear_table_trend_store_part_sql($1) USING $2;
...
COMMENT ON FUNCTION "trend_directory"."clear_table_trend_store_part"("table_trend_store_part_id" integer, "timestamp" timestamp with time zone) IS 'Removes all records of the specified timestamp from the table_trend_store_part and returns the removed record count.
';


CREATE FUNCTION "trend_directory"."transfer"("materialization" trend_directory.view_materialization, "timestamp" timestamp with time zone)
    RETURNS integer
AS $$
DECLARE
    columns_part text;
...
BEGIN
    ...
    EXECUTE format(
        'INSERT INTO trend.%I (created, job_id, %s) SELECT now(), 0, %s FROM %s WHERE timestamp = $1',
...
CREATE FUNCTION "trend_directory"."materialize"("materialization" trend_directory.view_materialization, "timestamp" timestamp with time zone)
...
BEGIN
    PERFORM trend_directory.clear_table_trend_store_part(
        $1.dst_trend_store_part_id, $2
    );

    result.row_count = trend_directory.transfer($1, $2);

    UPDATE trend_directory.view_materialization_state vms
    SET processed_fingerprint = vms.source_fingerprint
    WHERE vms.materialization_id = $1.id AND vms.timestamp = $2;

    RETURN result;
END;
$$ LANGUAGE plpgsql VOLATILE;

COMMENT ON FUNCTION "trend_directory"."materialize"("materialization" trend_directory.view_materialization, "timestamp" timestamp with time zone) IS 'Materialize the data produced by the referenced view of the view_materialization by clearing the timestamp in the target table_trend_store_part and inserting the data resulting from the view into it.
';


CREATE FUNCTION "trend_directory"."show_trends"("table_trend_store_part_id" integer)
    RETURNS SETOF trend_directory.trend_descr
AS $$
SELECT trend_directory.show_trends(table_trend_store_part)
...
CREATE FUNCTION "trend_directory"."to_char"(trend_directory.view_materialization)
    RETURNS text
AS $$
SELECT table_trend_store_part.name::text
FROM trend_directory.table_trend_store_part
WHERE table_trend_store_part.id = $1.dst_trend_store_part_id
$$ LANGUAGE sql STABLE STRICT;
...
CREATE FUNCTION "trend_directory"."dst_trend_store_part"(trend_directory.view_materialization)
    RETURNS trend_directory.table_trend_store_part
AS $$
SELECT * FROM trend_directory.table_trend_store_part WHERE id = $1.dst_trend_store_part_id;
$$ LANGUAGE sql STABLE;
...
COMMENT ON FUNCTION "trend_directory"."columns_part"(trend_directory.view_materialization) IS 'Return the comma separated, quoted list of column names to be used in queries';
...
CREATE CAST ("trend_directory"."view_materialization" AS text)
  WITH FUNCTION "trend_directory"."to_char"("trend_directory"."view_materialization");
...
BEGIN
    IF NEW.name <> OLD.name THEN
        FOR base_table_name IN
            SELECT trend_directory.base_table_name(table_trend_store_part)
            FROM trend_directory.table_trend
            JOIN trend_directory.table_trend_store_part ON table_trend.table_trend_store_part_id = table_trend_store_part.id
...
CREATE TRIGGER propagate_changes_on_trend_update
  AFTER UPDATE ON "trend_directory"."table_trend"
  FOR EACH ROW
...
AS $$
BEGIN
    PERFORM attribute_directory.deinit(OLD);
...
ALTER TABLE "trend_directory"."table_trend_store"
  ADD CONSTRAINT "table_trend_store_entity_type_id_fkey"
  FOREIGN KEY (entity_type_id)
  REFERENCES "directory"."entity_type" (id) ON DELETE CASCADE;

ALTER TABLE "trend_directory"."table_trend_store"
  ADD CONSTRAINT "table_trend_store_data_source_id_fkey"
  FOREIGN KEY (data_source_id)
  REFERENCES "directory"."data_source" (id);
...
ALTER TABLE "trend_directory"."view_trend_store"
  ADD CONSTRAINT "view_trend_store_entity_type_id_fkey"
  FOREIGN KEY (entity_type_id)
  REFERENCES "directory"."entity_type" (id) ON DELETE CASCADE;

ALTER TABLE "trend_directory"."view_trend_store"
  ADD CONSTRAINT "view_trend_store_data_source_id_fkey"
  FOREIGN KEY (data_source_id)
  REFERENCES "directory"."data_source" (id);
...
ALTER TABLE "trend_directory"."view_materialization"
  ADD CONSTRAINT "view_materialization_dst_trend_store_id_fkey"
  FOREIGN KEY (dst_trend_store_part_id)
  REFERENCES "trend_directory"."table_trend_store_part" (id) ON DELETE CASCADE;

ALTER TABLE "trend_directory"."function_materialization"
  ADD CONSTRAINT "function_materialization_dst_trend_store_id_fkey"
  FOREIGN KEY (dst_trend_store_id)
  REFERENCES "trend_directory"."table_trend_store" (id) ON DELETE CASCADE;

ALTER TABLE "trend_directory"."view_materialization_state"
  ADD CONSTRAINT "materialization_state_materialization_id_fkey"
  FOREIGN KEY (materialization_id)
  REFERENCES "trend_directory"."view_materialization" (id) ON DELETE CASCADE;
...
ALTER TABLE "trend_directory"."view_materialization_trend_store_link"
  ADD CONSTRAINT "materialization_trend_store_link_materialization_id_fkey"
  FOREIGN KEY (view_materialization_id)
  REFERENCES "trend_directory"."view_materialization" (id) ON DELETE CASCADE;
