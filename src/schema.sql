...
CREATE FUNCTION "alias_directory"."get_or_create_entity"("alias_type" name, "name" name)
    RETURNS directory.entity
AS $$
BEGIN
  PERFORM COALESCE(
      alias_directory.get_alias_type(alias_type),
      alias_directory.create_alias_type(alias_type)
  );
  RETURN COALESCE(
      alias_directory.get_entity_by_alias($1, $2),
      alias_directory.create_alias(directory.create_entity(format('%s=%s', $1, $2)), $1, $2)
  );
END;
$$ LANGUAGE plpgsql VOLATILE;
