...
CREATE OR REPLACE FUNCTION gis.existence_query(gis.existence_store, timestamp with time zone)
    RETURNS text
    LANGUAGE sql
AS $function$
    SELECT format('
    SELECT
        base.source_entity_id AS source_id,
        base.ho_entity_id AS handover_id,
        base.target_entity_id AS target_id,
        base.target_name::text,
        base.source_name::text,
        base.tag_name::text,
        base.direction,
        pa.start,
        (a.existence::bit(64) || b.existence::bit(64))::text,
        base.handover_tags
    FROM gis.handover_relation_base base
    JOIN gis.%I a ON a.entity_id = ho_entity_id
    JOIN gis.%I b ON a.entity_id = b.entity_id,
    gis.existence_store_partition AS pa
    WHERE pa.existence_store_id = %s AND pa.index = %s AND base.cell_entity_id = $1',
        gis.existence_table_name($1, gis.timestamp_to_existence_partition_index($1, $2) - 1),
        gis.existence_table_name($1, gis.timestamp_to_existence_partition_index($1, $2)),
        $1.id,
        gis.timestamp_to_existence_partition_index($1, $2) - 1
    );
$function$;


CREATE OR REPLACE FUNCTION gis.get_handovers_bits(cell_id integer)
    RETURNS TABLE(source_id integer, handover_id integer, target_id integer, target_name text, source_name text, tag_name text, direction text, existence_start timestamp with time zone, existence text, handover_tags character varying[])
    LANGUAGE plpgsql
AS $function$
BEGIN
  return query execute
    (select gis.existence_query(es, now()) from gis.existence_store es where es::text = 'existence_HandoverRelation')
  USING $1;
END
$function$;
