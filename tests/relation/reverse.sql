...
SELECT relation_directory.define_reverse('B->A','A->B');
...
SELECT throws_like($$SELECT relation_directory.define_reverse('B->C','C->B');$$, '%does not exist%', 'reverse table from non-existing table cannot be created');
