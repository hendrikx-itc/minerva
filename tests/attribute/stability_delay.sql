...
SELECT is(count(*), 1::bigint, 'sampled view materialization should be runnable more than stability_delay after max_modified')
FROM attribute_directory.sampled_view_materialization_runnable('2015-07-30 15:00+02') r
JOIN attribute_directory.sampled_view_materialization svm ON svm.id = r.id
WHERE svm::text = 'attribute.vsystem_state -> system-info_engine';

SELECT is(count(*), 0::bigint, 'sampled view materialization should not be runnable less than stability_delay after max_modified')
FROM attribute_directory.sampled_view_materialization_runnable('2015-07-30 14:24+02') r
JOIN attribute_directory.sampled_view_materialization svm ON svm.id = r.id
WHERE svm::text = 'attribute.vsystem_state -> system-info_engine';
