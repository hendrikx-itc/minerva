...
WHERE data_source.name = 'test1' AND entity_type.name = 'define_trendstore_entity_type';
...
SELECT trend_directory.define_trend_store_part(
    (trend_directory.get_trend_store('test2', 'define_trendstore_entity_type', '300 seconds'::interval)).id,
...
JOIN directory.data_source ON data_source.id = trend_store.data_source_id
JOIN directory.entity_type ON entity_type.id = trend_store.entity_type_id
WHERE data_source.name = 'test2' AND entity_type.name = 'define_trendstore_entity_type';
